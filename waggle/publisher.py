...
class BeehivePublisher(Publisher):

    def __init__(self, config):
        self.config = config

        self.parameters = pika.URLParameters(config['beehive_url'])
    ...
    def publish_message(self, message):
        print('---')
        print('Will publish to beehive', flush=True)
